...
<body>
	...
	<div class="bodyContainer">
		...
		<button onclick="a=document.getElementById('setname').value;b=document.getElementById('setvalue').value;if(a==null||a==undefined||a==''||b==null||b==undefined||b==''){alert('Both fields must be set!');return}settings.save(a,b)">Save</button><br/>
		<h2 class="heading">Load</h2>
		<label for="setnameload" class="formLabel">Name: </label>
		<input type="text" id="setnameload"><br>
		<button onclick="e=settings.load(document.getElementById('setnameload').value);h=document.getElementById('output').innerText;if(e!=null&&e!=undefined){h=e}else{h='none'}">Load</button><br/>
		...
		<button onclick="a=document.getElementById('output2');b=Object.keys(localStorage);if(b.length==0){a.innerHTML='None<br>'};for(c=0;c<b.length;b++){a.innerHTML+=(b[c]+'<br>')}">List</button><br/>
